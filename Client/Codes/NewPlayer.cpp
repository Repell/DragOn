...
HRESULT CNewPlayer::Ready_Object()
{
	...
	m_pTransform->Update_Component(ENGINE::Get_DeltaTime());
	m_pAdvance->Get_Transform()->Update_Component(ENGINE::Get_DeltaTime());
	Animate_FSM(110);
...
void CNewPlayer::Key_Check_Advance(const _double & TimeDelta)
{
	...
	else if (ENGINE::Key_Press(ENGINE::dwKEY_W))
	{
		...
		{
			//카메라 이동 -> 플레이어 이동으로 치환
			//m_vMoveDir = vAdvPos + (m_pTransform->Get_vLookDir() * 1.f);
			//if (m_pNaviMesh->Check_OnNaviMesh(&vAdvPos, &(vAdvDir * _CAMDIST)))
...
void CNewPlayer::Move_Func(const _double & TimeDelta)
{
	...
	if (m_ePlayerState == MOVE && m_ePlayerState != FIGHT_MOVE || m_ePlayerState == FIGHT_JUMP )
	{
		...
		//m_pAdvance->Set_Transform_Pos(ENGINE::INFO_POS, &vPos);
...
void CNewPlayer::Update_PlayerDir(const _double & TimeDelta)
{
	...
	case CNewPlayer::RIGHT:
		...
		m_pTransform->m_vAngle.y += fAngle * TimeDelta * fSpeed;
		break;
...
void CNewPlayer::Render_Object()
{
	...
	m_pShadow->Render_Shadow(&m_pTransform->m_matWorld, 0.35f);
...
void CNewPlayer::Reset_JumpStat()
{
	...
	m_pAdvance->Get_Transform()->m_fJump = 0.f;
	m_HoldTime = 1.0;
}
