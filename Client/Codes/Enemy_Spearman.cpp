...
#define _ATTACK_START			9
#define	_ATTACK_END				10
#define _HIT									19
#define _AIRBORNE						21
#define _KnockBack_A				27
#define _KnockBack_B				28
#define _KnockBack_C				29
#define _KnockBack_D				30
#define  _DEAD							31
#define _RISEUP							36	//기상
#define _AWAKEN						46	//사기 진작
...
VOID CEnemy_Spearman::Set_Behavior_Progress()
{
	...
	if (!m_bDead)	//안죽었을 때
	{
		...
		if (m_bSleep)	//생성 후 기본 대기 루프
			AiState = &CEnemy_Spearman::State_Awaken;
		//State Hit
		else if (m_bHit && !m_bKnockBack && !m_bAirborne)	//공격 명중 시, 경직
			AiState = &CEnemy_Spearman::State_Hit;
		//State KnockBack
		else if (m_bKnockBack)		//넉백 공격 받음
			AiState = &CEnemy_Spearman::State_KnockBack;
		//State Airbone
		else if (m_bAirborne && !m_bKnockBack)	//에어본 공격받음
			AiState = &CEnemy_Spearman::State_Airborne;
		//State Chase
		else if (!m_bAttack && !m_bHit && m_fDist < 10.f)	//플레이어 추적 시작
			AiState = &CEnemy_Spearman::State_Chase;
		//State Attack
		...
		if (m_fDist > 10.f && !m_bHit && !m_bKnockBack && !m_bAirborne && !m_bSleep)	//아무일도... 없엇따!
			AiState = &CEnemy_Spearman::State_Idle;
							...
							//State Dead	
		AiState = &CEnemy_Spearman::State_Dead;
...
VOID CEnemy_Spearman::State_Hit()
{
...
VOID CEnemy_Spearman::State_KnockBack()
{
...
VOID CEnemy_Spearman::State_Airborne()
{
...
VOID CEnemy_Spearman::State_Idle()
{
	Animate_FSM(_IDLE);
	m_bAttack = FALSE;
	m_bTwice = FALSE;
...
VOID CEnemy_Spearman::State_Chase()
{
...
VOID CEnemy_Spearman::State_Attack()
{
...
VOID CEnemy_Spearman::State_Dead()
{
	m_AttackTime += m_TimeDelta;
	m_bAttack = FALSE;
	m_pTransform->m_bAttackState = m_bAttack;
...
void CEnemy_Spearman::Check_EnemyGroup()
{
...
HRESULT CEnemy_Spearman::Add_Component()
{
	...
	{ FALSE, _vec3(0.f, 0.f, -130.f), _vec3{ _SCALE, _SCALE, _SCALE }, _vec3(90.f, 20.f, 0.f), _vec3(0.f, 0.f, 0.f), 40.f };
